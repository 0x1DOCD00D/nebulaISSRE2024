  ...
  @main def runIt = {
    new Frame {
      ...
      val gridPanel: GridPanel = new GridPanel(7, 2) {
        ...
        val optimizeButton: Button = new Button("Run Optimization") {
          reactions += {
            case event.ButtonClicked(_) =>
              if (actorJsonPath.nonEmpty && messagesJsonPath.nonEmpty && orchestratorJsonPath.nonEmpty) {
                Nebula.Main.optimize(actorJsonPath, messagesJsonPath, orchestratorJsonPath, clusteringJsonPath, monitoringJsonPath)
        ...
        val startNebulaButton: Button = new Button("Start Nebula") {
          reactions += {
            case event.ButtonClicked(_) =>
              if (actorJsonPath.nonEmpty && messagesJsonPath.nonEmpty && orchestratorJsonPath.nonEmpty) {
                Nebula.Main.startNebula(actorJsonPath, messagesJsonPath, orchestratorJsonPath, clusteringJsonPath, monitoringJsonPath, Seq())
                if (!systemRunning) {
                  systemRunning = true
        ...
        contents += new Label("Optimize:")
        contents += optimizeButton
