...
import NebulaScala3.Schema.{CinnamonActorMonitoringSchema, CinnamonMonitoringSchema, ClusterSchema, RemoteConfigSchema, ThresholdMonitoringSchema}
...
object ConfigCodeGenerator:
  ...
  def generateRemoteConfigCode(remoteConfigSchema: RemoteConfigSchema): String =
    s"""
       |akka {
       |  actor {
       |    provider = remote
       ...
       |    artery{
       |      canonical.hostname = ${remoteConfigSchema.host}
       |      canonical.port = ${remoteConfigSchema.port}
       |    }
       |  }
       |}
       |""".stripMargin
