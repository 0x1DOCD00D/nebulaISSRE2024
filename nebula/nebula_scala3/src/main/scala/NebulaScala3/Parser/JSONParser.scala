...
import NebulaScala3.Schema.{ActorSchema, CinnamonMonitoringSchema, ClusterSchema, GAResult, MessageSchema, OrchestratorSchema, RemoteConfigSchema}
...
object JSONParser {
   ...
   * Loads a schema from a JSON file.
   */
  def getRemoteSchemaFromJson(fileName: String): RemoteConfigSchema = {
    val inputStream = new FileInputStream(fileName)
    try {
      mapper.readValue(inputStream, classOf[RemoteConfigSchema])
    } finally {
      inputStream.close()
    }
  }
  
  def getGAResult(str: String): GAResult = {
    mapper.readValue(str, classOf[GAResult])
  }
